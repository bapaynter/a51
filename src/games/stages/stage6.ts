...
export const stage6: StageConfig = {
  ...
  commands: {
    ...
    select: (args: string[], _state: GameState): CommandResult => {
      ...
      if (
        query.includes("black_hangars") &&
        query.includes("hangar_number=18")
      ) {
        return {
          success: true,
          output:
